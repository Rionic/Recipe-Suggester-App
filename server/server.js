...
app.post('/api/search', async (req, res) => {
    try {
        ...
        const response = await axios.get(`https://api.spoonacular.com/recipes/complexSearch?${queryString}&number=1&apiKey=${API_KEY}`);
...
app.get('/api/recipe/ingredients', async (req, res) => {
    try {
        ...
        const response = await axios.get(`https://api.spoonacular.com/recipes/informationBulk?ids=${recipeIds.split(',')[0]}&apiKey=${API_KEY}`);
        ...
        console.log('recipeDetails', recipeDetails[0].extendedIngredients);
